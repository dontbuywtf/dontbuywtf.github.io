    ...
    for (let i = 400; i < worldWidth - 500; i += 250) {
        ...
        if (Math.random() < 0.2 + (num * 0.08)) {
            ...
            if (eType < 0.5) {
                ...
                enemies.push({ x: i + 20, y: py - 35, w: 30, h: 25, vx: maxiSpeed, range: maxiRange, startX: i + 20, type: 'maxi', label: "HODL" });
            } else {
                let botSpeed = -2 - (num * 1.2);
                let botRange = 100 + (num * 20);
                enemies.push({ x: i, y: py - 85, w: 25, h: 25, vx: botSpeed, range: botRange, startX: i, type: 'bot', label: "MEV" });
            }
        }
    }
...
function drawPlatform(ctx, p) {
    ...
    ctx.beginPath(); ctx.rect(p.x + 2, p.y + 2, p.w - 4, p.h - 4); ctx.clip();
    ctx.fillStyle = "rgba(0, 255, 65, 0.4)";
    ctx.font = "bold 10px 'monospace'";
    if (p.w < 200) {
        let txt = xrpLore[Math.floor((p.x) % xrpLore.length)];
        ctx.textAlign = "center"; ctx.fillText(txt, p.x + p.w / 2, p.y + 13);
    } else {
        ctx.textAlign = "left";
        for (let x = 10; x < p.w; x += 100) {
            let txt = xrpLore[Math.floor((p.x + x) % xrpLore.length)];
            ctx.fillText(txt, p.x + x, p.y + 13);
        }
    }
    ctx.restore(); ctx.save();
    ctx.strokeStyle = "#32e685"; ctx.lineWidth = 1; ctx.strokeRect(p.x, p.y, p.w, p.h);
    ctx.restore();
}

function drawFlag(ctx, g) {
    ctx.save();
    let cx = g.x + 20; let bottomY = 360; let topY = 200;
    ctx.strokeStyle = "#32e685"; ctx.lineWidth = 4;
    ...
    ctx.beginPath(); ctx.moveTo(cx, topY); ctx.lineTo(cx + 65, topY + 20 + wave); ctx.lineTo(cx, topY + 45);
    ctx.closePath(); ctx.fill();
    ctx.fillStyle = "#fff"; ctx.font = "bold 14px Arial"; ctx.fillText("X", cx + 12, topY + 28 + (wave/2));
    ctx.restore();
}
...
function win() {
    ...
    if (currentLevel === 10) {
        ...
        const k = "Vk9JRC1LRVktNTg5NTg5NTg5"; const e = "ZG9udGJ1eXRoaXNAcHJvdG9uLm1l"; 
        rewardContainer.innerHTML = `
            <div style="margin-top:20px; border: 1px dashed #00ff41; padding: 20px; background: rgba(0,255,65,0.1);">
                ...
                <div style="background:#000; padding:10px; font-size:1.4rem; letter-spacing:3px; border:1px solid #00ff41; color:#00ff41;">${atob(k)}</div>
                <p style="margin-top:15px; font-size:0.9rem;">TARGET: ${atob(e)}</p>
            </div>`;
    } else {
        rewardContainer.innerHTML = ""; nextBtn.classList.remove('hidden');
...
window.onkeydown = e => {
    if (e.code === 'ArrowUp' && player.jumpsLeft > 0 && gameState === 'PLAYING' && !isDying) {
...
function update() {
    ...
    uiAnims.forEach((anim, i) => {
        anim.y -= 1; anim.life--;
        if (anim.life <= 0) uiAnims.splice(i, 1);
    });
    ...
    if (!isDying) {
        ...
        tokens.forEach(t => {
            if (!t.collected && player.x < t.x + 40 && player.x + player.w > t.x && player.y < t.y + 20 && player.y + player.h > t.y) {
                t.collected = true;
                if (t.type === '1up') {
                    lives++; 
                    uiAnims.push({ x: player.x, y: player.y, text: "+1UP", life: 60, color: "#00ff41" });
                } else {
                    score[t.type]++;
                }
                updateUI();
...
function draw() {
    ...
    uiAnims.forEach(anim => {
        ctx.save();
        ctx.globalAlpha = anim.life / 60;
